...
struct cumsum {
    ...
    cumsum() : m_previousValue(0) {}

    return_type operator()(Iterator &current) const {
        if (NA) {
            if (na< typename traits::ctype< return_type >::type >::is_na(*current)
                || na< typename traits::ctype< return_type >::type >::is_na(m_previousValue)) {
                m_previousValue = na< return_type >::VALUE();
            } else {
                m_previousValue = *current + m_previousValue;
            }
        } else {
            m_previousValue = *current + m_previousValue;
        }
    ...
    inline dims_t result_dims(const dims_t &rhs) const {
        if (rhs.second == 0) {
            return dims_t(rhs.first, 0);
        } else {
            return dims_t(rhs.first * rhs.second, 0);
        }
    }
private:
    mutable return_type m_previousValue;
...
struct cummin {
    ...
    cummin() : m_previousValue(std::numeric_limits< return_type >::max()) {}
    ...
    return_type operator()(Iterator &current) const {
        if (NA) {
            if (na< typename traits::ctype< return_type >::type >::is_na(*current)
                ...
                m_previousValue = std::min(*current, m_previousValue);
            }
        } else {
            m_previousValue = std::min(*current, m_previousValue);
        }
...
struct cummax {
    ...
    cummax() : m_previousValue(std::numeric_limits< return_type >::min()) {}
    ...
    return_type operator()(Iterator &current) const {
        if (NA) {
            if (na< typename traits::ctype< return_type >::type >::is_na(*current)
                ...
                m_previousValue = std::max(*current, m_previousValue);
            }
        } else {
            m_previousValue = std::max(*current, m_previousValue);
        }
